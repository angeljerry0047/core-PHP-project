...
if(isset($_GET['action']) && $_GET['action']=='active')
{
	 ...
	 $sql="update one_way_user set status='inactive' where id='".$item_id."'";
	$res=mysql_query($sql);
	$_SESSION['msg']=message('Status update successfully',1);
	//header('Location: '.basename(__FILE__));
	//exit();
   ...
   if($u=='c')
		{
			...
			$sql="select * from one_way_user a where a.name<>'' and a.id<>1 and  account_type='".$t."' and a.status='active' order by `user_id` ASC";

		}
	else
		{
			 $sql="select * from one_way_user a where a.name<>'' and `parent_id`='".$parentid."'  and account_type='".$t."' and a.status='active'  and a.id <>1 order by `user_id` ASC";
		}
...
<style type="text/css">
 ...
 <script language="javascript">
   function del(aa)
   {
      ...
      if (a)
      {
        location.href="own_list_user.php?cid="+ aa +"&action=delete&t=<?php echo $t?>&u=<?php echo $u?>"
      }  
   } 
   function inactive(aa)
   {
     var a=confirm("Are you sure, you want to Active this user?")
      if (a)
      {
       location.href="own_list_user.php?cid="+ aa +"&action=inactive&t=<?php echo $t?>&u=<?php echo $u?>"
	  }
   ...
   function active(aa)
   {
   		var a=confirm("Are you sure, you want to Inactive this user?")
      if (a)
      {
     
       location.href="own_list_user.php?cid="+ aa +"&action=active&t=<?php echo $t?>&u=<?php echo $u?>"
	  }
...
<tr><td><table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>">
    <?php 
	  ...
	  ?>
	<td width="12%" align="center" class="light_text_bold">Name </td>
	
    <td width="11%" align="center" class="light_text_bold">User ID </td>
   
	<td width="11%" align="center" class="light_text_bold">Expiry Date</td>
	
	<td width="11%" align="center" class="light_text_bold">Password </td>
	
	
	
	<td width="55%" align="center" class="light_text_bold">Option</td>
  </tr>
  <?php
...
?>
  <tr class="<?php echo ++$flag%2==0?"alternate_color1":"alternate_color2";?>" onclick="javascript:location.href='#response.php?mid=<?php echo $row->color_id;?>';" onmouseover="this.className='hover_color'" onmouseout="this.className='<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>'">
   <td class="light_text" align="center"><p style="padding-left:5px;"><?php if($t!="reseller" && $row->parent_id=="1") echo "* " ?><?php echo ($row->name);?>&nbsp;
</p></td>
...
<?php if($t=='reseller'){?>
<input name="button" type="button"   bgcolor="<?php echo $color?>" class="butt1" style="background:<?php echo $color?>" onclick="window.location.href='own_list_retailer_user.php?t=retailer&u=r&p=<?php echo $row->id?>&rid=<?php echo $row->user_id?>'" value="View Retailer" /><?php } ?>

<?php if($type=='Company'){?>
<input name="button" type="button"   bgcolor="<?php echo $color?>" class="butt1" style="background:<?php echo $color?>" onclick="window.location.href='own_list_user_group.php?game_id=<?php echo $row->id; ?>'" value="View Group" />&nbsp;<?php } ?>
<input name="button4" type="button" class="butt1"  style="background:<?php echo $color?>" onclick="window.location.href='owm_add_user.php?game_id=<?php echo $row->id; ?>&action=edit&t=<?php echo $t?>&u=<?php echo $u?>&pg=<?php echo basename(__FILE__)?>'" value="Edit" />
...
<?php
		...
		{
?>
<input name="button" type="button" class="butt1" style="background:<?php echo $color?>" onclick="javascript:active(<?php echo $row->id?>);" value="Inactive" />
<?php 
		}
		else 
		{
?>
        <input name="button3" type="button" class="butt1" style="background:<?php echo $color?>" onclick="javascript:inactive(<?php echo $row->id?>);" value="Active" />
<?php 
		}
?> </td>
  </tr>
