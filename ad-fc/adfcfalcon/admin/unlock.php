...
if(isset($_GET['action']) && $_GET['action']=='unlock')
{
	 ...
	 $sql="update one_way_user set islogin=0 where id='".$userid."'";
	$res=mysql_query($sql);
	$_SESSION['msg']=message('User have been unloked successfully',1);
...
//-----------------------------------Generate Sql----------------------
			 ...
			 $q="select * from one_way_user a where a.name<>'' and a.islogin=1 order by `user_id` ASC";
...
<tr><td><table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>">
    <?php 
	  ...
	  ?>
	<td width="10%" align="center" class="light_text_bold">Name </td>
	
    <td width="9%" align="center" class="light_text_bold">User ID </td>
   
	<td width="9%" align="center" class="light_text_bold">Expiry Date</td>
	
	<td width="9%" align="center" class="light_text_bold">Password </td>
	
	
	
	<td width="26%" align="center" class="light_text_bold">Last Login </td>
	<td width="37%" align="center" class="light_text_bold">Option</td>
  </tr>
  <?php
...
<td class="light_text" align="center"><?php echo ($row->lastlogin);?></td>
<td colspan="4" align="center" class="light_text">&nbsp;&nbsp;
<input name="button4" type="button" class="butt1"  style="background:<?php echo $color?>" onclick="window.location.href='unlock.php?userid=<?php echo $row->id; ?>&action=unlock'" value="Unlock" /></td>
  </tr>
