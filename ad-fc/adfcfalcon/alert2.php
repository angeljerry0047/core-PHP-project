...
//substr($e,8,2)."-".substr($e,5,2)."-".substr($e,0,4);

  $tdate=date('Y-m-d',mktime(0,0,0,substr($e,5,2), substr($e,8,2)+$talert,substr($e,0,4)));
   $sdate=date('Y-m-d',mktime(0,0,0,substr($e,5,2), substr($e,8,2)+$salert,substr($e,0,4)));
   
  //  $tdate1=date('Y-m-d',mktime(0,0,0,date('m'), date('d')+$talert,date('Y')));
 //  $sdate1=date('Y-m-d',mktime(0,0,0,date('m'), date('d')+$salert,date('Y')));


  $dt=date('d-m-Y',mktime(0,0,0,substr($e,5,2), substr($e,8,2),substr($e,0,4)));
    $dt5=date('Y-m-d',mktime(0,0,0,substr($e,5,2), substr($e,8,2),substr($e,0,4)));
...
/*if(mysql_num_rows(mysql_query("select * from `one_way_alert` where `date`='$dt5'"))!=0)
			echo "You cant give alert twice for perticular date";
	exit();*/
