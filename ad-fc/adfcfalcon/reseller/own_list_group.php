...
if(isset($_GET['action']) && $_GET['action']=='delete')
{
	...
	mysql_query("delete from `one_way_group` where id='$item_id'");
...
$q="select * from one_way_group a where a.name<>'' order by `id` ASC";
...
function show()
{
 ...
 <script language="javascript">
   function del(aa)
   {
      ...
      if (a)
      {
        location.href="own_list_group.php?cid="+ aa +"&action=delete"
      }  
   } 
   function inactive(aa)
   {
     var a=confirm("Are you sure, you want to Not Archive this item?")
      if (a)
      {
       location.href="own_list_group.php?cid="+ aa +"&action=inactive"
	  }
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td align="left" class="body_title_style"  bgcolor="<?php echo $color?>">Product Group List </td>
...
<tr><td><table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>">
	...
	<td width="31%" align="center" class="light_text_bold">Description</td>
	
	<td width="47%" align="center" class="light_text_bold">Option</td>
  </tr>
  <?php
...
?>
  ...
  <td class="light_text"><p style="padding-left:5px;"><?php echo ($row->details);?>&nbsp;
	...
	<input name="button4" type="button" class="butt1" style="background:<?php echo $color?>"   onclick="window.location.href='own_list_group_user.php?game_id=<?php echo $row->id; ?>&amp;action=add'" value="View User" />&nbsp;
	<input name="button3" type="button" class="butt1"  style="background:<?php echo $color?>"  onclick="window.location.href='own_list_group_product.php?game_id=<?php echo $row->id; ?>&amp;action=add'" value="View Product" />
      &nbsp;
 <input name="button" type="button" class="butt1" style="background:<?php echo $color?>"   onclick="window.location.href='owm_add_group.php?game_id=<?php echo $row->id; ?>&action=edit'" value="Edit" />
&nbsp;
