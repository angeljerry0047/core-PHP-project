...
if(isset($_POST['submit']))
{
	...
	if($action=='add')		//------------------ADD--------------------
	{
		if(mysql_num_rows(mysql_query("select * from `one_way_group` where `name`='$name'"))!=0)
		
		$_SESSION['msg']=message("Sorry, the Group is already exist. Please choose another.",2);
		
		else
			{
		
				echo $sql="INSERT INTO `one_way_group` (`name`,`details`) VALUES ('$name','$description')";
			...
			if(mysql_affected_rows()>0)
			{
				...
				$_SESSION['msg']=message(" Please enter another Group.",3);
				header('Location:owm_add_group.php');
				exit();
	...
	elseif($action=='edit')		//------------------EDIT--------------------
	{
			...
			//move_uploaded_file($tmp_name,$dest);	
			 ...
			 $sql="update `one_way_group` set `name`='$name',`details`='$description' where `id`='$game_id'";
			//echo $sql;
			
				mysql_query($sql) or die(mysql_error());
				$mysql_affected_rows+=mysql_affected_rows();
			//$_SESSION['msg'].=message(" Testimonials successfully updated.",3);
			$q="select * from `one_way_group` where id='$game_id'";
			$rs=mysql_query($q);
			$row=mysql_fetch_array($rs);
			if($row['id']!=0)
			{
				$_SESSION['msg']=message(" Product Update Sccessful. ",3);
				header('Location:own_list_group.php');
				//exit();
			}
			/*else
			{
...
elseif($action=='edit')
{
	$row=mysql_fetch_object(mysql_query("select * from `one_way_group` where id='$game_id'"));
	$name=$row->name;
...
<tr>
<td width="705" height="57" class="body_title_style"  bgcolor="<?php echo $color?>"><?php echo $action=='edit'?"Edit":"Add";?>  Group </td>
</tr>
<tr>
...
<tr>
    ...
    <td width="53%"><textarea name="description" cols="45" rows="10"> <?php echo $description; ?></textarea>
	</td>
</tr>
