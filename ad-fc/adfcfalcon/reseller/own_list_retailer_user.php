...
//-----------------------------------Generate Sql----------------------
   ...
   if($u=='c')
		...
		{
			 $sql="select * from one_way_user a where a.name<>'' and `parent_id`='".$parentid."'  and account_type='".$t."'   and a.id <>1 order by `user_id` ASC";
		}
...
<div align="LEFT" style=" font-size:28px; ;">Reseller ID :<?php echo $_REQUEST['rid'];?>&nbsp;</div>
<br />
...
<?php if($t=='reseller'){?>
<input name="button" type="button"   bgcolor="<?php echo $color?>" class="butt1" style="background:<?php echo $color?>" onclick="window.location.href='own_list_retailer_user.php?t=retailer&u=r&p=<?php echo $row->id?>'" value="View Retailer" />&nbsp;<?php } ?>
