...
if(isset($_GET['action']) && $_GET['action']=='inactive')
{
	 ...
	 $sql="select * from one_way_user a where a.name<>'' and a.status='active'  and `parent_id`='".$_SESSION['id']."'";
	 $f=mysql_query($sql);
	 $num=mysql_num_rows($f);
	 if($num>=$_SESSION['max'])
	 {	  $_SESSION['msg']=message("Sorry You Can Active Only ".$_SESSION['max']." users",2);
          $show="no";
	 }
	 else
	 {
			...
			$item_id=$_GET['cid'];
			$sql="update one_way_user set status='active' where id='".$item_id."'";
			$res=mysql_query($sql);
			$_SESSION['msg']=message('status update successfully',1);
			//header('Location: '.basename(__FILE__));
			//exit();
...
//-----------------------------------Generate Sql----------------------
 	...
 	if($type=='Company')
		{
			$sql="select * from one_way_user a where a.name<>'' and a.id <>1 and a.status='inactive' order by `user_id` ASC";
		}
	else
		{
			 $sql="select * from one_way_user a where a.name<>'' and `parent_id`='".$parentid."' and a.status='inactive'  and a.id <>1 order by `user_id` ASC";
		}
...
function show()
{
 ...
 <script language="javascript">
   function del(aa)
   {
      ...
      if (a)
      {
        location.href="own_list_inactive_user.php?cid="+ aa +"&action=delete"
      }  
   } 
   function inactive(aa)
   {
     var a=confirm("Are you sure, you want to Active this user?")
      if (a)
      {
       location.href="own_list_inactive_user.php?cid="+ aa +"&action=inactive"
	  }
   ...
   function active(aa)
   {
      ...
      if (a)
      {
       ...
       location.href="own_list_inactive_user.php?cid="+ aa +"&action=active"
	  }
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td align="left" class="body_title_style"  style="background:<?php echo $color?>" >User List </td>
			<td width="170" align="right" class="body_title_style"  style="background:<?php echo $color?>" >&nbsp;<?php echo $total_count;?> Record(s) found &nbsp;</td>
		  </tr>
	  </table></td></tr>
<tr><td><table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>">
    <?php 
	  ...
	  ?>
	<td width="8%" align="center" class="light_text_bold">Name </td>
	
    <td width="10%" align="center" class="light_text_bold">User ID </td>
   
	<td width="13%" align="center" class="light_text_bold">Account Type</td>
	...
	<td width="46%" align="center" class="light_text_bold">Option</td>
  </tr>
  <?php
...
?>
 ...
 <td class="light_text" align="center"><p style="padding-left:5px;"><?php echo ($row->account_type);?>&nbsp;
</p></td>
...
<?php if($type=='Company'){?>
<input name="button" type="button" class="butt1" style="background:<?php echo $color?>"   onclick="window.location.href='own_list_user_group.php?game_id=<?php echo $row->id; ?>'" value="View Group" />&nbsp;<?php } ?>
 <input name="button" type="button" class="butt1" style="background:<?php echo $color?>"   onclick="window.location.href='owm_add_user.php?game_id=<?php echo $row->id; ?>&action=edit'" value="Edit" />
