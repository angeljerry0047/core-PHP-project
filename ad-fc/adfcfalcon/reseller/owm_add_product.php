...
if(isset($_POST['submit']))
{
	...
	if($action=='add')		//------------------ADD--------------------
	{
		...
		if(mysql_num_rows(mysql_query("select * from `one_way_product` where `name`='$name'"))!=0)
		
		$_SESSION['msg']=message("Sorry, the Product is already exist. Please choose another.",2);
		
		else
			{
				$sql="INSERT INTO `one_way_product` (`name`,`details`,`code`,`aname`,`acode`) VALUES ('$name','$description','$code','$aname','$acode')";
				...
				$_SESSION['msg']=message("Product has been added sucessfully.",2);
		        header('Location:owm_add_product.php');
		}	
			//	exit();
	}		
	...
	elseif($action=='edit')		//------------------EDIT--------------------
	{
			...
			//move_uploaded_file($tmp_name,$dest);	
			 ...
			 $sql="update `one_way_product` set `name`='$name',`code`='$code',`aname`='$aname',`acode`='$acode',`details`='$description' where `id`='$game_id'";
			//echo $sql;
			...
			$q="select * from `one_way_product` where id='$game_id'";
			$rs=mysql_query($q);
			$row=mysql_fetch_array($rs);
			if($row['id']!=0)
			{
				$_SESSION['msg']=message("Product has been updated sucessfully.",2);
				header('Location:own_list_product.php');
				//exit();
			}
			/*else
			{
...
elseif($action=='edit')
{
	$row=mysql_fetch_object(mysql_query("select * from `one_way_product` where id='$game_id'"));
	$name=$row->name;
	$code=$row->code;
	$aname=$row->aname;
	$acode=$row->acode;
...
<tr>
<td width="705" height="57" class="body_title_style"   bgcolor="<?php echo $color?>"><?php echo $action=='edit'?"Edit":"Add";?> Product </td>
</tr>
<tr>
...
<form name="ff" method="post" action="<?php echo basename(__FILE__);?>" onsubmit="return check()" enctype="multipart/form-data">
  ...
  <tr>
    <td class="normal_text" colspan="2" align="center">Code :</td>
    <td><input name="code" type="text" id="code" value="<?php echo $code; ?>" /></td>
  </tr>
  <tr>
  ...
  <tr>
    <td class="normal_text" colspan="2" align="center">Arabic Code : </td>
    <td><input name="acode" type="text" id="acode" value="<?php echo $acode; ?>" dir="rtl" /></td>
  </tr>
  <tr>
    <td class="normal_text" colspan="2" align="center">Arabic Name : </td>
    <td><input type="text" value="<?php echo $aname; ?>" name="aname"   dir="rtl"></td>
  </tr>
 

<tr>
    <td width="47%" class="normal_text" colspan="2" align="center">Description:</td>
    <td width="53%"><textarea name="description" cols="45" rows="10"> <?php echo $description; ?></textarea>	</td>
</tr>
