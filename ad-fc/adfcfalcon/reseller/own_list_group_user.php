...
if(isset($_POST['submit']))
{
	...
	if(mysql_num_rows(mysql_query("select * from `one_way_user_group` where `user_id`='$user_id' and `group_id`='$groupid'"))!=0)
		
		$_SESSION['msg']=message("Sorry, the User is already exist. Please choose another.",2);
		
		else
			{
				 $sql="insert into `one_way_user_group`(`user_id`,`group_id`) VALUES ('$user_id','$groupid')";
				 $res=mysql_query($sql);
		     }
...
if(isset($_GET['action']) && $_GET['action']=='delete')
{
	...
	$_SESSION['msg']=message('The User is deleted from the group successfully',1);
...
//-----------------------------------Generate Sql----------------------
	...
	$q="select b.id,a.user_id,a.account_type,b.group_id from one_way_user a,one_way_user_group b where a.name<>'' and 	
				a.id=b.user_id and b.group_id='".$groupid."' order by a.id ASC";
...
function show()
{
 ...
 <script language="javascript">
   function del(aa,gid)
   {
      ...
      if (a)
      {
        location.href="own_list_group_user.php?cid="+ aa +"&game_id="+gid+"&action=delete"
      }  
   } 
...
<form name="ff" method="post" action="<?php echo basename(__FILE__);?>"  enctype="multipart/form-data">
  ...
  <tr>
<td width="705" height="57" class="body_title_style" colspan="4"    bgcolor="<?php echo $color?>">Add User</td>
</tr>
<tr>
<td colspan="5" height="10">&nbsp;</td>
</tr>
  <tr>
    <td width="47%" class="normal_text" colspan="2" align="right">Select User:</td>
    <td width="26%">
	<select name="user" id="user">
      <option value="" selected="selected">Select User ID</option>
      <?php
	$user=mysql_query("select * from `one_way_user` where id not in (select a.id from one_way_user a,one_way_user_group b where a.name<>'' and 	
				a.id=b.user_id and b.group_id='".$groupid."' order by a.id ASC)  and id<>1 order by user_id ");
	while($Rsuser=mysql_fetch_object($user))
	{
	?>
      <option value="<?php echo $Rsuser->id;?>" <?php if($userId==$Rsuser->id) echo "selected"?>>
	  <?php echo $Rsuser->user_id;?>	  </option>
      <?php
	}
	?>
    </select>	</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td width="669" align="left" class="body_title_style"    bgcolor="<?php echo $color?>">User List of
			<?php 
			  $res=mysql_query("select * from `one_way_group` where id='".$groupid."'");
			  $row=mysql_fetch_array($res);
			  echo $row['name'];
...
<tr><td><table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>">
    <?php 
	  ...
	  ?>
	<td width="29%" align="center" class="light_text_bold">Name </td>
	<td width="35%" align="center" class="light_text_bold">Account Type</td>
	<td width="36%" align="center" class="light_text_bold">Option</td>
  </tr>
  <?php
...
?>
  <tr class="<?php echo ++$flag%2==0?"alternate_color1":"alternate_color2";?>" onclick="javascript:location.href='#response.php?mid=<?php echo $row->color_id;?>';" onmouseover="this.className='hover_color'" onmouseout="this.className='<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>'">
   <td class="light_text"><p style="padding-left:5px;" align="center"><?php echo ($row[1]);?>&nbsp;
</p></td>
  <td class="light_text"><p style="padding-left:5px;" align="center"><?php echo ($row[2]);?>&nbsp;
</p></td>
