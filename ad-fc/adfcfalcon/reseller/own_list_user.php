...
//-----------------------------------Generate Sql----------------------
 	...
 	if($type=='Company')
		{
			$sql="select * from one_way_user a where a.name<>'' and a.id <>1 and a.status='active' order by `user_id` ASC";
		}
	else
		{
			 $sql="select * from one_way_user a where a.name<>'' and `parent_id`='".$parentid."' and a.status='active'  and a.id <>1 order by `user_id` ASC";
		}
/*if($keyword!="")
{
	$key=filter($keyword);
...
<style type="text/css">
 ...
 <script language="javascript">
   function del(aa)
   {
      ...
      if (a)
      {
        location.href="own_list_user.php?cid="+ aa +"&action=delete"
      }  
   } 
   function inactive(aa)
   {
     var a=confirm("Are you sure, you want to Active this user?")
      if (a)
      {
       location.href="own_list_user.php?cid="+ aa +"&action=inactive"
	  }
...
<tr><td><table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tr class="<?php echo $flag%2==0?"alternate_color1":"alternate_color2";?>">
    ...
    <td width="9%" align="center" class="light_text_bold">User ID </td>
