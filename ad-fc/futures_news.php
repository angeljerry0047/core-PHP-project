...
$file = fopen ("http://futures.fxstreet.com/futures/news/afx/latestfuturesnews.asp", "r");
if (!$file) {
    echo "<p></p>\r\n";
}
else
{
  ...
  $pos = strpos ($sResponse, '<table width="100%" border="0" cellspacing="0" cellpadding="3" valign="top">');
  $sResponse = substr ($sResponse, $pos + 1);
  $pos = strpos ($sResponse, '<table width="100%" border="0" cellspacing="0" cellpadding="3" valign="top">');
  $sResponse = substr ($sResponse, $pos);
  $pos = strpos ($sResponse, '<tr class=taulafonsfosc>');
  $sResponse = substr ($sResponse, 0, $pos);
  $sResponse .= "</table>";
  $sResponse = str_replace ('src="/', 'src="http://futures.fxstreet.com/', $sResponse);
  $sResponse = str_replace ('href="singleNew.asp', 'target="_blank" href="http://futures.fxstreet.com/futures/news/afx/singleNew.asp', $sResponse);
  //$sResponse = preg_replace ('|<tr class=taulafonsfosc>[\s\S]*?</tr>|i', '', $sResponse);
